const CACHE_NAME = "ras-sudr-baz-v31";
const CORE = [
  ...
  "./assets/v21.js?v=21","./assets/v22.js?v=22","./assets/v23.js?v=23","./assets/v24.js?v=24","./assets/v25.js?v=25","./assets/v26.js?v=26","./assets/v27.js?v=27","./assets/v28.js?v=28","./assets/v29.js?v=29","./assets/v30.js?v=30","./assets/v31.js?v=31",
...
self.addEventListener("fetch",event=>{
  ...
  if(isAppCode){event.respondWith(fetch(req,{cache:"no-store"}).then(res=>{if(res&&res.ok){const copy=res.clone();caches.open(CACHE_NAME).then(c=>c.put(req,copy))}return res}).catch(async()=>{const cached=await caches.match(req);if(cached)return cached;if(req.mode==="navigate")return caches.match("./index.html");throw new Error("offline")}));return}
  event.respondWith(caches.match(req).then(cached=>{const refresh=fetch(req).then(res=>{if(res&&res.ok){const copy=res.clone();caches.open(CACHE_NAME).then(c=>c.put(req,copy))}return res}).catch(()=>cached);return cached||refresh}))
});